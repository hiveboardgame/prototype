...
import { acceptGameChallenge, GameChallenge, getGameChallenge, usePlayer, UserData } from 'hive-db';
...
function getExpansionDescription(gameType: string): string {
  const expansionPieces = []
  if (gameType.includes('M')) {
    expansionPieces.push('Mosquito');
  }
  if (gameType.includes('L')) {
    expansionPieces.push('Ladybug');
  }
  if (gameType.includes('P')) {
...
const ChallengeInfoBox = (challenge: GameChallenge) => {
  ...
  return (
    <>
      <div className='bg-slate-50 rounded p-4 pt-3'>
        ...
        <ChallengeInfoBox {...challenge}></ChallengeInfoBox>
